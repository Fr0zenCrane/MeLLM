...
class OFAEncoder(OFAPreTrainedModel):
    ...
    def __init__(self, config: OFAConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        # self.mask_embed_increment = nn.Embedding(2, embed_dim, padding_idx=0)
    ...
    def forward_embedding(
        ...
        if token_embedding is None:
            mask_idx = torch.where(input_ids == 50264, 1, 0)
            # mask_embeds_incre = self.mask_embed_increment(mask_idx)
            token_embedding = self.embed_tokens(input_ids) # + mask_embeds_incre
            # print(self.mask_embed_increment)
